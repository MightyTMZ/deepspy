...
| **Feature matrix and diff summary with Claude** (`src/intel/extract.ts`): Opus 5 behind a forced tool schema, taxonomy in `taxonomy/saas.json`, rows without evidence rejected, status never `verified_working`; `POST /runs/:id/extract`, auto-extract after a run when the key is set; `scripts/extract.ts` | Implemented | 4 tests (A10, A11, summary); live: Spotify 16 rows with evidence incl. regional pricing, Ornn 5 rows |
| **Stagehand on Steel**: Steel cannot load Stagehand's extension from disk, so `src/steel/stagehand-extension.ts` uploads it once through Steel's extensions API and installs it into model-driven sessions; the bridge attaches by id | Attaches; extract and act verified in isolation; Tom's reveal on top still loses the page. Opt in with `PERISCOPE_STAGEHAND=1` | probes on Sept 13 morning; not in the demo path |
...
3. **Stagehand on top of the deterministic pass (Tom).** It now attaches to Steel through the uploaded extension and `extract`/`act` work in isolation, but inside `runReveal` the Playwright page is reported closed once Stagehand runs. Reproduce with `PERISCOPE_STAGEHAND=1` and the Ornn command. Everything in the demo runs without it; the matrix comes from `src/intel/extract.ts`, not Stagehand.
