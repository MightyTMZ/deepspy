...
// Shared contracts — frozen Friday night. Changes need all three owners.
...
export interface Observation {
  ...
  vantage: Vantage;
  perception: "dom" | "a11y" | "screenshot"; // which rung produced it
...
export interface LeaseRequest {
  ...
  profileId?: string;
  accountRef?: string;
  purpose: "surface" | "reveal" | "borders" | "walker" | "setup";
}

export interface WallDetected {
  // B or C raises, C handles
  jobId: string;
  sessionId: string;
  wall:
    | "captcha"
...
export interface HandoffEvent {
  jobId: string;
  viewerUrl: string;
  wall: WallDetected["wall"];
  generation: number;
  state: "awaiting_human" | "resumed" | "abandoned";
}
...
export type Event =
  ...
  | {
      type: "counter";
      data: { competitor: string; url: string; missed: number | "uncertain" };
    }
...
// Internal event sink interface — B and C call this, Ayan implements
